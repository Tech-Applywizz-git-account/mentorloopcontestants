  ...
  const fetchDashboardData = async () => {
    ...
    if (mentorsData && mentorsData.length > 0) {
      ...
      for (const mentor of mentorsData) {
        ...
        if (mentor.comments) {
          commentsMap[mentor.id] = mentor.comments;
        }
      }
          ...
          <div className="bg-background border rounded-lg p-6 w-full max-w-md relative">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button 
                variant="outline" 
                onClick={async () => {
                  // Save just the comment without changing engagement status or awarding points
                  const success = await handleEngagementUpdate(
                    engagementPopup.mentorId, 
                    engagementPopup.status, 
                    engagementComment
                  );
                  if (success) {
                    // Update the existing comments display to show the saved comment
                    setExistingComments(prev => ({
                      ...prev,
                      [engagementPopup.mentorId]: engagementComment
                    }));
                    alert('Comment saved successfully!');
                  } else {
              ...
              <Button 
                onClick={async () => {
                  // Save comment first
                  await handleEngagementUpdate(
                    engagementPopup.mentorId, 
                    engagementPopup.status, 
                    engagementComment
                  );
                  
                  // Then handle point awarding and engagement status update
                  await handleAwardPoints(
                    engagementPopup.mentorId, 
                    engagementPopup.status, 
                    engagementPopup.mentor
